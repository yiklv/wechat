...
var HOST_URI_MINAPPER_PLUS_JSON = 'https://' + domain + '/wp-json/minapper-plus/v1/';
...
// 获取置顶的文章
 ...
 //获取首页滑动文章
 getSwiperPosts: function (args) {
  var url = HOST_URI_MINAPPER_PLUS_JSON;
  url +='post/swipe?apptype='+args;
  return url;
},
  ...
  getCategories: function (data) {
    ...
    var url =HOST_URI_MINAPPER_PLUS_JSON +'category?apptype=wx';
    if(data.openid)
    {
  ...
  getNewComments: function () {
      ...
      var url = HOST_URI_MINAPPER_PLUS_JSON;    
      url =url+ 'comment/new?limit='+pageCount+'&page=1';
       return url;
  },
  ...
  //获取评论
  getWeixinComment: function (data) {
      var openid= data.openid;
      var apptype=data.apptype;
      var url = HOST_URI_MINAPPER_PLUS_JSON;
      return url + 'comment/get?openid=' + openid+"&apptype="+apptype;
  },    
  ...
  getContentFirstImage: function (content){
    ...
    var src ="../../images/src/post_default_img.jpg";
    if(arrReg){   
      src=arrReg[1];
    }
    return src;  
 ...
 getTopHotPosts(flag){      
  ...
  if(flag ==1)
  {
      url +="post/hotpostthisyear?apptype=wx"
  }
  else if(flag==2)
  {
      url += "post/pageviewsthisyear?apptype=wx"
  }
  else if (flag == 3) {
      url += "post/likethisyear?apptype=wx"
  }
  else if (flag == 4) {
      url += "post/praisethisyear?apptype=wx"
  }
  ...
  getMyLikeUrl(data) {
      ...
      var apptype=data.apptype;
      var url = HOST_URI_MINAPPER_PLUS_JSON;
      url += "post/mylike?openid=" + openid+"&apptype="+apptype;
      return url;
  },
  ...
  getMyPraiseUrl(data) {
    ...
    var apptype=data.apptype;
      var url = HOST_URI_MINAPPER_PLUS_JSON;
      url += "post/mypraise?openid=" + openid+"&apptype="+apptype;
      return url;
  },
